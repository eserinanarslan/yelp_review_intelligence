...
class SentimentModelTrainer:
    """
    Training component for the sentiment classification model.
    ...
    def stratified_sample(
        ...
        """
        Create a stratified sample while preserving the target distribution.

        This keeps experiments faster while maintaining class balance.
        """
    ...
    def get_experiments() -> dict[str, dict[str, Any]]:
        """
        Define feature configurations for ablation experiments.

        Each experiment tests whether additional context improves
    ...
    def build_pipeline(
        ...
        """
        Build a scikit-learn pipeline for text and optional numeric features.
        ...
        # Add numeric preprocessing only when numeric features are provided
        if numeric_cols:
            transformers.append(
                (
                    "numeric",
    ...
    def run_ablation(
        ...
        """
        Run ablation experiments across multiple feature configurations.

        This helps quantify the contribution of review text, categories,
        tips, location and numeric features.
        """
        target_col = self.config.raw["model"]["target_col"]

        # Use a stratified sample to keep experimentation efficient
        train_sample = self.stratified_sample(
            train_df,
            target_col,
            int(self.config.raw["data"]["train_sample_size"]),
        ...
        # Train and evaluate each feature configuration
    ...
    def tune_best_model(
        ...
        """
        Tune the best ablation experiment using GridSearchCV.

        The final model is selected based on cross-validation performance
        and then evaluated on the untouched test set.
        """
        target_col = self.config.raw["model"]["target_col"]

        # Use the same stratified sampling strategy for tuning
        train_sample = self.stratified_sample(
            train_df,
            target_col,
            int(self.config.raw["data"]["train_sample_size"]),
        ...
        # Load Grid Search parameters from config
        raw_grid = self.config.raw["model"]["grid_search"]["param_grid"]

        param_grid = {
            "preprocessor__text__ngram_range": [
                tuple(x)
                for x in raw_grid["preprocessor__text__ngram_range"]
            ],
            "preprocessor__text__min_df": raw_grid[
                "preprocessor__text__min_df"
            ],
            "preprocessor__text__max_features": raw_grid[
        ...
        grid_search = GridSearchCV(
            ...
            cv=int(grid_cfg["cv"]),
            n_jobs=int(grid_cfg["n_jobs"]),
            verbose=int(grid_cfg["verbose"]),
        )
        ...
        # Add class probabilities when available
    ...
    def feature_importance(
        ...
        """
        Extract most important TF-IDF terms for each sentiment class.

        Logistic Regression coefficients make the model interpretable.
        """
        vectorizer = model.named_steps["preprocessor"].named_transformers_[
            "text"
        ]
        ...
        for class_idx, class_name in enumerate(classifier.classes_):
            ...
            tables[class_name] = pd.DataFrame(
                {
    ...
    def confusion_matrix_df(
        ...
        """
        Create raw and normalized confusion matrices as dataframes.
        """
        cm = confusion_matrix(
            y_true,
            y_pred,
